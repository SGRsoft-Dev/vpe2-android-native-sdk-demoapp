...
#   없으면  → REMOTE : AAR 전용 배포 저장소(raw GitHub Maven, com.navercloud.vpe:player) 소비
...
#   scripts/select-sdk.sh remote           # AAR 전용 배포 저장소
# ---------------------------------------------------------------------------
set -euo pipefail
...
EOF
  ...
  remote)
    rm -f "$ENV_FILE"
    echo "▶ SDK 참조 모드 → REMOTE (dist raw-maven com.navercloud.vpe:player)"
    echo "  .local.env 제거됨. AAR 전용 배포 저장소(raw GitHub Maven)를 소비합니다."
    ;;
  status)
    if [ -f "$ENV_FILE" ]; then
      ...
      echo "현재 모드: REMOTE (.local.env 없음 → dist raw-maven)"
    fi
    ;;
  *)
